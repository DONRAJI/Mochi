...
/**
 * DELETE /api/push/subscribe — 리마인더 구독 해지. 인증 필수.
 * `endpoint`를 주면 그 구독만(설정에서 끔), 안 주면 **내 웹 구독 전부**(앱이 채널을 가져갈 때).
 */
export async function DELETE(request: Request) {
  try {
    ...
    if (parsed.data.endpoint) await removeSubscription(userId, parsed.data.endpoint);
    else await clearWebSubscriptions(userId);
    return ok({ done: true });
  } catch (error) {
    return toErrorResponse(error);
  }
