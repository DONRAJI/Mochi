 ...
 *
 * **대표 메뉴만** 보여준다. 처음엔 장소 안의 전부를 페이지로 줬더니 카페만 4천 개가 넘어(439쪽)
 * 찾을 수도, 제안이라 할 수도 없었다. 여러 곳에서 파는 메뉴 = 대중적인 메뉴라, 파는 곳이
 * MIN_POPULARITY 이상인 메뉴 중 많이 파는 순으로 PLACE_TOP_N개를 추린 뒤 가벼운 순으로 보여준다.
 *
 ...
 *   (된장국·나물 같은 곁들이, 현미밥 같은 맨밥은 빠진다). 피자는 어디에도 넣지 않는다 — '한 판' 단위라
 *   조각 기준이 없다.
...
/** 장소마다 보여줄 대표 메뉴 수 — 한 페이지 10개면 최대 4쪽. */
export const PLACE_TOP_N = 40;

/**
 * 제안에 넣을 최소 대중성(파는·만드는 서로 다른 곳의 수). 한 곳에서만 파는 메뉴는 뺀다.
 * 2026-09 실측(합친 뒤 2곳 이상): 카페 295 · 빵집 205 · 식사 119 · 버거·치킨 33 · 편의점 1,657.
 */
...
/**
 * '밥'으로 끝나지만 식사가 아니라 밥 한 공기 — 급식 데이터(여러 대상별 식단)에 흔해서, 대중성으로 뽑으면
 * 자장면·비빔밥과 나란히 식사 한 끼 상위에 오른다.
 */
export const PLAIN_RICE = [
  "강낭콩밥",
  "완두콩밥",
  "팥밥",
  "조밥",
...
/** 식사 끝말로 끝나지만 한 끼가 아닌 것 — 고구마맛탕은 '탕'으로 끝나는 간식이다. */
export const NOT_MEAL_SUFFIXES = ["맛탕"] as const;
...
export const NON_MEAL_CATEGORIES: readonly string[] = [
  ...CAFE,
  ...BAKERY,
  ...FASTFOOD,
  ...EXCLUDED,
...
const PLAIN_RICE_SET = new Set<string>(PLAIN_RICE);

/** 사전 항목이 어느 장소에 속하는지(대중성과 무관한 분류만). 어디에도 안 속하면 null. */
export function placeOf(entry: {
  ...
  if (
    entry.kcal >= MEAL_MIN_KCAL &&
    !PLAIN_RICE_SET.has(entry.name) &&
    MEAL_SUFFIXES.some((s) => entry.name.endsWith(s)) &&
    !NOT_MEAL_SUFFIXES.some((s) => entry.name.endsWith(s))
  ) {
    return "meal";
  }
  return null;
